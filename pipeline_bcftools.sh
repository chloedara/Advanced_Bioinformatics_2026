...
# 3. Variant calling
# Ensure shared libraries are installed prior: conda install -y -c conda-forge libopenblas
# Use a 'pipe' (|) to connect two commands: 'mpileup' and 'call'.

# 1. 'bcftools mpileup' calculates the genotype likelihoods from the BAM file.
#    -f: The reference genome used for alignment.
#    -Ou: Outputs uncompressed BCF format to save time and disk space during the transfer to the next command.
# 2. 'bcftools call' uses those likelihoods to identify actual variants.
#    --ploidy 2: Explicitly defines the sample as human (diploid).
#    -m: Uses the multiallelic caller, which is better at identifying sites with multiple alternative alleles.
#    -v: Output variants ONLY (excludes non-variant/reference positions) to keep the VCF file manageable.
#    -o: Specifies the final output path for the VCF file.
bcftools mpileup -f ~/ngs_assignment/dnaseq/data/reference/hg19.fa $ALIGNED_DIR/NGS0001_sorted_filtered.bam -Ou | bcftools call --ploidy GRCh37 -m -v -o $RES_DIR/NGS0001_bcftools.vcf

# 4. Quality filter the variants
vcffilter -f "QUAL > 20" $RES_DIR/NGS0001_bcftools.vcf > $RES_DIR/NGS0001_bcftools_filtered.vcf
...
head -n -1 $RES_DIR/NGS0001_bcftools_filtered.vcf | bedtools intersect -header -wa -a stdin -b ~/ngs_assignment/dnaseq/data/annotation.bed > $RES_DIR/NGS0001_bcffinal_filtered.vcf

# 6. Compress and Index final filtered file
bgzip $RES_DIR/NGS0001_bcffinal_filtered.vcf
tabix -p vcf $RES_DIR/NGS0001_bcffinal_filtered.vcf.gz
...
./convert2annovar.pl -format vcf4 $RES_DIR/NGS0001_bcffinal_filtered.vcf.gz > $RES_DIR/NGS0001_bcffinal_filtered.avinput

# Run Annovar table function
./table_annovar.pl $RES_DIR/NGS0001_bcffinal_filtered.avinput humandb/ -buildver hg19 -out $RES_DIR/NGS0001_bcf_annovar -remove -protocol refGene,ensGene,clinvar_20180603,exac03,dbnsfp31a_interpro -operation g,g,f,f,f -otherinfo -nastring . -csvout
...
awk -F',' '$6 ~ /exonic/ && $21 == "."' $RES_DIR/NGS0001_bcf_annovar.hg19_multianno.csv > $RES_DIR/NGS0001_bcf_prioritized_variants.csv
